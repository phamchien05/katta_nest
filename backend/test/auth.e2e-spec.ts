...
describe('Auth (e2e)', () => {
  ...
  it('đăng nhập đúng / sai mật khẩu / email không tồn tại', async () => {
    ...
    await agent()
      .send({ email: 'an@example.com', password: 'secret123' })
      .expect(200);
    const wrongPass = await agent()
      .send({ email: 'an@example.com', password: 'sai-sai-sai' })
      .expect(401);
    const noUser = await agent()
      .send({ email: 'khong-co@example.com', password: 'secret123' })
      .expect(401);
  ...
  it('user cũ từ bản Laravel (hash $2y$) đăng nhập được', async () => {
    ...
    const laravelHash = bcrypt
      .hashSync('secret123', 12)
      .replace(/^\$2b\$/, '$2y$');
    fake.rows.push({
      id: 7n,
      name: 'Cũ',
  ...
  it('GET /auth/me: token giả/hỏng bị từ chối', async () => {
    await request(app.getHttpServer())
      .get('/api/auth/me')
      .set('Cookie', 'katta_token=khong.phai.jwt')
      .expect(401);
  });
